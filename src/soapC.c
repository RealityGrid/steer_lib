...
SOAP_SOURCE_STAMP("@(#) soapC.c ver 2.7.6e 2006-08-07 14:15:09 GMT")
...
SOAP_FMAC3 int SOAP_FMAC4 soap_out_sws__AttachResponse(struct soap *soap, const char *tag, int id, const struct sws__AttachResponse *a, const char *type)
{
	...
	soap_out_reg__steerMessage(soap, "ReG_steer_message", -1, &a->ReG_USCOREsteer_USCOREmessage, "");
	soap_element_end_out(soap, tag);
	return SOAP_OK;
}
...
{
	soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, a, SOAP_TYPE_reg__steerMessage), type);
	soap_out_reg__supp_cmds(soap, "Supported_commands", -1, &a->Supported_USCOREcommands, "");
	soap_element_end_out(soap, tag);
	return SOAP_OK;
}
...
{	int i, j;
	...
	if (soap->body && !*soap->href)
	{
		...
		{	soap_new_block(soap);
			for (a->__size = 0; ; a->__size++)
			{	p = (struct reg__Command *)soap_push_block(soap, sizeof(struct reg__Command));
				if (!p)
					return NULL;
...
{
	soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, a, SOAP_TYPE_reg__Command), type);
