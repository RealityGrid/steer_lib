...
sub Attach {
    ...
    if($SGS::serviceData{$steerStatusSDE} eq "DETACHED" &&
       $SGS::serviceData{$appStatusSDE} ne "NOT_STARTED"){
...
sub AppDetach {
    ...
    if($SGS::serviceData{$SGS::steerStatusSDE} ne "DETACHED"){
...
sub AppStop {
    ...
    if($SGS::serviceData{$SGS::appStatusSDE} ne "STOPPED"){

	SetServiceData(" ", $SGS::appStatusSDE, "STOPPED");

        # If no steerer is attached then we're now free to die
	if($SGS::serviceData{$SGS::steerStatusSDE} ne "ATTACHED"){
